...
public class Baek_1267 {
    public static void main(String[] args) throws IOException {
        ...
        if(youngSik(arr) < minSik(arr))
            bw.write("Y " + youngSik(arr));
        else if(youngSik(arr) > minSik(arr))
            bw.write("M " + minSik(arr));
        else
            bw.write("Y M " + youngSik(arr));
    ...
    private static int youngSik(int[] arr) {
        return IntStream.of(arr).map(i -> (i / 30 + 1) * 10).sum();
    }

    private static int minSik(int[] arr) {
        return IntStream.of(arr).map(i -> (i / 60 + 1) * 15).sum();
    }
}
