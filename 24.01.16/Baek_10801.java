...
public class Baek_10801 {
    public static void main(String[] args) throws IOException {
        BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
        BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(System.out));

        int[] scoresA = new int[10];
        int[] scoresB = new int[10];
        ...
        for(int i = 0; i < 10; i++) {
            scoresA[i] = Integer.parseInt(inputA[i]);
            scoresB[i] = Integer.parseInt(inputB[i]);
        ...
        bw.write(scoreA > scoreB ? "A" : scoreA < scoreB ? "B" : "D");
        bw.flush();
        bw.close();
    }
}
